...
- Have a program connect and send strings to ```ws://{IPAddress}:{Port}``` where {IPAddress} will be that of the machine running TTS Voice Wizard (if it is the same machine it would be "localhost"). And the specified {Port} (9008 by default)
